...
//Clicking the Facebook link opens a new tab.
  const [newPage] = await Promise.all([//ensures Playwright waits for the tab and performs the click simultaneously.
    context.waitForEvent('page'), // wait for new tab
    page.click("(//a[contains(@href,'facebook')])[1]") // action that opens it
  ]);
  await newPage.waitForLoadState();//Ensures the new tab is fully loaded before interaction.
